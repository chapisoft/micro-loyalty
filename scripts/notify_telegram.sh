...
if [ -f "$ROOT_DIR/deploy/micro-loyalty/.env" ]; then
    set -a
    [ -s "$ROOT_DIR/deploy/micro-loyalty/.env" ] && eval "$(grep -E '^(TELEGRAM_BOT_TOKEN|TELEGRAM_CHAT_ID)=' "$ROOT_DIR/deploy/micro-loyalty/.env" || true)"
    set +a
elif [ -f "$ROOT_DIR/deploy/natcash/.env" ]; then
    set -a
    [ -s "$ROOT_DIR/deploy/natcash/.env" ] && eval "$(grep -E '^(TELEGRAM_BOT_TOKEN|TELEGRAM_CHAT_ID)=' "$ROOT_DIR/deploy/natcash/.env" || true)"
    set +a
