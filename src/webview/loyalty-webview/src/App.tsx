...
export const App: React.FC = () => {
  const [currentTab, setCurrentTab] = useState<'HOME' | 'WHEEL' | 'VOUCHERS'>('HOME');
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-16 font-sans">
      {currentTab === 'WHEEL' && <LuckyWheelPage onBack={() => setCurrentTab('HOME')} />}
      {currentTab === 'VOUCHERS' && <UserVoucherPage onBack={() => setCurrentTab('HOME')} />}
      {currentTab === 'HOME' && (
          ...
          <div className="p-4 space-y-4 max-w-md mx-auto w-full flex-1">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <span className="text-[11px] text-amber-600 font-bold mt-0.5">Quay thưởng trúng quà</span>
              </div>

              <div
                onClick={() => setCurrentTab('VOUCHERS')}
                className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200/70 flex flex-col items-center text-center cursor-pointer hover:border-indigo-400 active:scale-98 transition"
              >
                <div className="w-11 h-11 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center mb-2 shadow-inner">
                  <Ticket className="w-6 h-6" />
                </div>
                <span className="font-bold text-sm text-slate-800">Kho Voucher</span>
                <span className="text-[11px] text-indigo-600 font-bold mt-0.5">Phiếu giảm giá đã lưu</span>
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-slate-200 px-6 py-2 flex justify-around items-center z-40 max-w-md mx-auto">
        <button
          onClick={() => setCurrentTab('HOME')}
          className={`flex flex-col items-center space-y-1 transition ${
            currentTab === 'HOME' ? 'text-indigo-600 font-bold' : 'text-slate-400'
          }`}
        >
        ...
        <button
          onClick={() => setCurrentTab('WHEEL')}
          className={`flex flex-col items-center space-y-1 transition ${
            currentTab === 'WHEEL' ? 'text-indigo-600 font-bold' : 'text-slate-400'
          }`}
        >
        ...
        <button
          onClick={() => setCurrentTab('VOUCHERS')}
          className={`flex flex-col items-center space-y-1 transition ${
            currentTab === 'VOUCHERS' ? 'text-indigo-600 font-bold' : 'text-slate-400'
          }`}
        >
